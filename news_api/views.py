...
class ArticleViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        """
        Handlers the Journalist 'None' rule for the reader field and
        bypasses approval if 'No Publisher' (Independent) is selected.
        """
        user = self.request.user
    ...
    def perform_update(self, serializer):
        """Set published_at when an editor approves an article
...
def signup_view(request):
    """Handle registration with dynamic role selection."""
    if request.method == "POST":
